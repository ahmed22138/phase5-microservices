...
export function setLeaderStatus(leader: boolean): void {
  isLeader = leader;
}
...
router.get('/health', (_req: Request, res: Response) => {
  const health: HealthStatus = {
    ...
    checks: [
      ...
      { name: 'leader', status: isLeader ? 'pass' : 'fail', message: isLeader ? 'Is leader' : 'Not leader (standby)' }
    ]
  };
...
router.get('/ready', async (_req: Request, res: Response) => {
  ...
  checks.push({
    ...
    status: 'pass',
    message: isLeader ? 'Active leader' : 'Standby replica'
  });
