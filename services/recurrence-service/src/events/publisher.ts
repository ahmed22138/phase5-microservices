/**
 * Dapr Event Publisher for Recurrence Service
 */
...
const logger = pino({ name: 'recurrence-service-event-publisher' });
...
export class EventPublisher {
  ...
  constructor(daprHost?: string, daprPort?: string) {
    this.daprClient = new DaprClient({
      ...
      daprPort: daprPort ?? process.env.DAPR_HTTP_PORT ?? '3503',
    });
    this.pubsubName = process.env.PUBSUB_NAME ?? 'pubsub';
  }

  async publishRecurrenceEvent<T>(
    eventType: string,
    taskId: string,
    userId: string,
    payload: T,
    ...
    const event: BaseDomainEvent<T> = {
      ...
      payload,
      metadata: {
        serviceName: 'recurrence-service',
    ...
    try {
      await this.daprClient.pubsub.publish(this.pubsubName, Topics.RECURRENCE_EVENTS, event);
      ...
      logger.info({
        ...
        taskId,
        correlationId,
      }, 'Recurrence event published successfully');
    } catch (error) {
      logger.error({
        eventId,
        eventType,
        taskId,
        error,
      }, 'Failed to publish recurrence event');
      throw error;
    }
  }
}
